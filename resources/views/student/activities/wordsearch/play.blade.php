...
<div class="wrap">
    <div class="card">
        ...
        <p class="scoreline">
            ...
            / {{ $maxScore }} · Palabras: <strong id="words-count">{{ $foundCount }}</strong>/{{ $words->count() }}
        </p>

        <div id="result" class="result"></div>

        <div class="layout">
            <div class="grid" id="grid" style="grid-template-columns: repeat({{ $wordsearch->columns }}, 2rem); --cols: {{ $wordsearch->columns }};">
            ...
            <div class="hints">
                ...
                @foreach ($words as $word)
                    <div class="hint {{ in_array($word->id, $foundIds) ? 'found' : '' }}" data-word-id="{{ $word->id }}">
                        <span>{{ $word->word }}</span>
