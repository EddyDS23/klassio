...
class ActivityController extends Controller
{
    ...
    public function teacherStore(StoreActivityRequest $request, int $id): RedirectResponse
    {
        $class = SchoolClass::findOrFail($id);
        Gate::authorize('create', [Activity::class, $class]);
        $activity = $this->activityService->create($class, $request->validated());
        return redirect()->route('teacher.activities.edit', $activity->id)->with('success', 'Actividad creada correctamente.');
    }
